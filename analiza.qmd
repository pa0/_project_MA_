...
data = data %>%
  mutate(test_stat = ifelse(!is.na(test_list.stat), names(test_list.stat), NA),
...
# d = beta / s_pooled lub beta = beta * sy/sx (x = sqrt(n1 − n1**2/N)/ (N − 1)) i dopiero d = ... (1 - beta standaryzowane, 2 - beta niewystandardowane)
# lub wziąć p i znaleźć t
...
data2 = data %>%
  relocate(c(n, n_prosoc, n_con), .after = my_SMD)
...
# 1 - n = n1 + n2, n1 != n2 
# 2 - n = n1, n2 = NA
# 3 - n != n1 + n2    (sprawdzić w artykule czemu)
# 4 - n1 = n2 = NA

# sprawdzić co powinno być n
# zmienić CI na beta na T i to na d
# niektóre jeśli mają m, sd policzyć z nich zamiast ze statystyk

data %>%
  mutate(test_stat = ifelse(!is.na(test_list.stat), names(test_list.stat), NA),
         my_SMD = case_when(n_con == n_prosoc & test_stat == 'z'   ~ effectsize::z_to_d(test_list.stat, n = n)$d,
                            ...
                            sum(is.na(c(n_con, n_prosoc))) == 0 & n_con != n_prosoc & test_stat == 't'   ~ esc::esc_t(test_list.stat, grp1n = n_prosoc, grp2n = n_con)$es,  
                            sum(is.na(c(n_con, n_prosoc))) == 0 & n_con != n_prosoc & test_stat == 'f'   ~ esc::esc_f(test_list.stat, grp1n = n_prosoc, grp2n = n_con)$es,
                            sum(is.na(c(n_con, n_prosoc))) == 0 & n_con != n_prosoc & test_stat == 'chi' ~ esc::esc_rpb(test_list.stat, grp1n = n_prosoc, grp2n = n_con, totaln = n)$es,
                            TRUE ~ NA))

# powyższe w teorii zajmuje się przypadkami 0,1,3; w teorii, bo dla n1 != n2 coś tutaj nie zadziałało, do poprawy
