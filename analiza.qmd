...
data = data %>%
  mutate(test_stat = case_when(effect_coef == 'r' ~ 'r',
         ...
         SMD_R = case_when(test_stat == 'r'                                                                  ~ esc::esc_rpb(effect_size, totaln = n)$es,
                           ...
                           if_all(matches('^(m_|sd_)(.*)(_prosoc|_con)$'), ~ !is.na(.x)) & is.na(test_stat)  ~
...
k = data %>%
  filter(if_all(matches('(_con|_prosoc)$'), ~ is.na(.x)) & test_stat != 'r')    # te trzeba sprawdzić, czy to dane zależne
View(k)
