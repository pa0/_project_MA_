...
data = data %>%
  mutate(test_stat = case_when(effect_coef == 'r' ~ 'r',
         ...
         SMD_R = case_when(test_stat == 'r'                                    ~ esc::esc_rpb(effect_size, totaln = n)$es,                                     
                           ...
                           TRUE ~ NA),                                                                                                             # tutaj muszę zmienić chyba, bo może to być chi z McNemara 
...
# Nad poniższymi wariancjami muszę zastanowić się kolejny raz
...
data = data %>% 
  mutate(SMD_var  = case_when((n_con == n_prosoc | (is.na(n_con) & is.na(n_prosoc))) & test_stat == 'chi' ~ SMD_R**2/abs(test_list.stat),
         ...
         W_n        = case_when(!is.na(n_con) & !is.na(n_prosoc) ~ n,
                                is.na(n_con) & is.na(n_prosoc)   ~ n * 2),           # ? - mogę tu też tak zignorować, że to zależne? [nieistotne - i tak tego nie używam]
...
#rstatix::mcnemar_test(matrix(c(794, 86, 150, 570), nrow = 2))
#effectsize::cohens_g(matrix(c(794, 86, 150, 570), nrow = 2))
