...
# pozostałe 4 są całe testy i statystyki do uzupełnienia

# dla przypadku 2
data = data %>%                                                                
    mutate(n_con = ifelse(cite == 'morschheuser2017how_a', NA, n_con),
           test_list.stat = structure(ifelse(cite == 'morschheuser2017how_a', unname(effect_size / ((test_list.stat_2 - test_list.stat)/2/qt(0.975, n - 2))), test_list.stat),
                                       names = ifelse(cite == 'morschheuser2017how_a', 't', names(test_list.stat))),
           test_list.stat_2 = NULL)
# pozostałe 2 do sprawdzenia

# dla przypadków 0,1 (w tym 3,4) 
data = data %>%
  mutate(test_stat = ifelse(!is.na(test_list.stat), names(test_list.stat), NA),
         SMD_R = case_when(if_all(matches('(_con|_prosoc)$'), ~ !is.na(.x)) ~
