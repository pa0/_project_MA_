...
data = data %>%
  mutate(test_stat = case_when(effect_coef == 'r' ~ 'r',
         ...
         SMD_R = case_when(test_stat == 'r'                                    ~ esc::esc_rpb(effect_size, totaln = n)$es,                                     
                           ...
                           is.na(n_con) & is.na(n_prosoc) & test_stat == 'chi' ~ log(max(c((38 + 14)/(23 + 10)), (23 + 10)/(38 + 14))) * sqrt(3) / pi, # z OR = max(b/c, c/b) z tabeli krzyżowej
                           TRUE ~ NA),                                                                                                            
...
# Nad poniższymi wariancjami (ze znakami zapytania) muszę zastanowić się kolejny raz
...
data = data %>% 
  mutate(SMD_var  = case_when(is.na(n_con) & is.na(n_prosoc) & test_stat == 'chi' ~ (1/(38 + 14) + 1/(23 + 10)) * 3 / pi**2,
                              n_con == n_prosoc & test_stat == 'chi'              ~ SMD_R**2/abs(test_list.stat),
...
metafor::rma(yi = abs(data$SMD_R), vi = data$SMD_var)  # SMD = 0.39, słaby związek
