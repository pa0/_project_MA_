...
# dla przypadku 2
data = data %>%                                                                
    mutate(test_list.stat = structure(ifelse(cite == 'morschheuser2017how_a', unname(effect_size / ((test_list.stat_2 - effect_size)/qt(0.975, n - 3))), test_list.stat),    # chyba dobry test jest (β = 0.476, p < .001 (i chyba bet jest 3), ale tu brakuje SE
...
# ogólne poprawy
data = data %>%
  mutate(n = case_when(cite == 'parsons2019prosocial_a'       ~ 71,
                       cite == 'jin2017when_c'                ~ 93,
                       ...
                       TRUE                                   ~ n)) %>%
  rows_update(data.frame(cite  = c('kaufman2015psychologically_a', 'kaufman2015psychologically_c', 'boduszek2019prosocial_a', 'boduszek2019prosocial_b'),
                         n_con = c(18, 16, 86, 86), n_prosoc = c(19, 16, 86, 86)), by = 'cite') %>%
  ...
  mutate(test_list.se   =   if_else(cite == 'wu2019promoting_a', c('se' = 0.07), test_list.se),
         test_list.stat = case_when(cite == 'wu2019promoting_a' | cite == 'shoshani2021video_a' ~ c('t' = effect_size/test_list.se),
                                    cite == 'lobel2016relation_a'                               ~ c('t' = sign(effect_size) * qt(1 - test_list.p/2, n - 9)),
                                    TRUE                                                        ~ test_list.stat))
