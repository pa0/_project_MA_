...
# ogólne poprawy
data = data %>%
  mutate(n = case_when(#cite == 'wu2019promoting'              ~ 131,     # PK dlaczego? grały tylko osoby z 2 grup 28+34
...
data = data %>%
  ...
  mutate(
    ...
    SMD_R = case_when(
      test_stat == 'r'                                    ~ esc::esc_rpb(effect_size, totaln = n)$es,
      ...
      n_con != n_prosoc &
        test_stat == 't'                ~ esc::esc_t(test_list.stat, grp1n = n_prosoc, grp2n = n_con)$es,
      n_con != n_prosoc &
        test_stat == 'f'                ~ esc::esc_f(test_list.stat, grp1n = n_prosoc, grp2n = n_con)$es,
      n_con != n_prosoc &
        test_stat == 'chi'              ~ esc::esc_rpb(
          effectsize::chisq_to_phi(abs(test_list.stat), n = n, adjust = F)$phi,
          grp1n = n_prosoc,
          grp2n = n_con
        )$es,
      n_con == n_prosoc &
        test_stat == 'z'                ~ effectsize::z_to_d(test_list.stat, n = n)$d,
      n_con == n_prosoc &
      ...
      is.na(n_con) &
        is.na(n_prosoc) &
        test_stat == 'chi' ~ effectsize::oddsratio_to_d(max(c((38 + 14) / (23 + 10)), (23 + 10) / (38 + 14))), # z OR = max(b/c, c/b) z tabeli krzyżowej
      TRUE ~ NA_real_
    ),
    SMD_R = ifelse(
      cite == 'beene2015effect_a' |
        cite == 'gordon2018powering_a',
      NA,
      SMD_R
    )
  ) %>%
...
data = data %>% 
  mutate(SMD_var  = case_when(test_stat == 'r'                                    ~ 4/((1 - effect_size**2) * (n - 1)),
                              n_con == n_prosoc & test_stat == 'chi'              ~ SMD_R**2/abs(test_list.stat),
                              is.na(n_con) & is.na(n_prosoc) & test_stat == 'chi' ~ (1/(38 + 14) + 1/(23 + 10)) * 3 / pi**2,
